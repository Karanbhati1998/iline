...
const AddOffer = () => {
  ...
  const handleSubmit = () => {
    ...
    if (handleValidation()) {
      dispatch(addOffer(data)).then((res) => {
        if (res?.payload?.code == 200) {
          toastService.success("Vehicle category offer added successfully");
          dispatch(getOffersList({ categoryId: state, page }));
  ...
  const handleDelete = () => {
    dispatch(deleteOffer({id})).then(res=>{
      if(res?.payload?.code==200){
        toastService.success("Offer deleted successfully");
        dispatch(getOffersList({ categoryId: state, page }));
        setUpdateState(initialState);
      }else{
        toastService.error("Failed to delete offer");
      }
    })
  };
  const handleClose = () => {
  ...
  return (
    <>
      ...
      {updateModal && (
        <UpdateOffer handleCloseUpdateModal={handleCloseUpdateModal} data={data} />
      )}
      {deleteModal && (
        <DeleteVechileModal
