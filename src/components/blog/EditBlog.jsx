...
import { editBlog } from '../../features/slices/blogSlice';
...
const EditBlog = () => {
  ...
  const { author, description, errors, image, title, imageLoader,id } = iState;
  ...
  const handleSubmit = () => {
    ...
    if (handleValidation()) {
      dispatch(editBlog(data)).then((res) => {
        if (res?.payload?.code == 200) {
          toastService.success("Blog edit successfully");
          setUpdateState(initialState);
          navigate("/blogManagement");
        } else {
          toastService.error("Blog edit failed");
        }
      });
    }
  };
