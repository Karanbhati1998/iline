...
const TotalRevenuefromILineDriverTab = ({ categoryId }) => {
  ...
  useEffect(() => {
    ...
    if (categoryId) {
      dispatch(getIlineRevenueListDownload(data)).then((res) => {
  ...
  const handleReset = () => {
    ...
    dispatch(
      getIlineRevenueList({ categoryId, revenueType: "ILINE", page: 1 })
    );
  };
  const handleApply = () => {
    const data = {
        ...
        <div className="Small-Wrapper">
          ...
          <div className="TableList mt-4">
            <table style={{ width: "150%" }}>
              ...
              <tbody>
                {ilineRevenueList?.result?.[0]?.paginationData?.map(
                  (res, i) => {
                    return (
                      <tr>
                        <td>{i + 1 + (totalRevenueIlinePage - 1) * 10}</td>
...
export default TotalRevenuefromILineDriverTab;
